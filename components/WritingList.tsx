...
export function WritingList({ posts }: WritingListProps) {
  ...
  const filteredPosts = useMemo(() => {
    return posts.filter(
      (post) => activeTags.length === 0 || activeTags.some((tag) => post.categories?.includes(tag))
    )
  }, [posts, activeTags])
        ...
        <div style={{ position: 'relative', flexShrink: 0 }}>
          <button
            ...
            style={{
              ...
              borderRadius: '0.5rem',
              background:
                filterOpen || activeFilterCount > 0 ? 'hsl(var(--foreground))' : 'transparent',
              color:
        ...
        {filteredPosts.length === 0 ? (
          ...
          <ul style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {filteredPosts.map((post) => {
              const isNew =
                !post.favourite && now - new Date(post.date).getTime() < 30 * 24 * 60 * 60 * 1000
              return (
                <li
                  key={post.slug}
                  style={{
                ...
                >
                  <span
                    style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem', minWidth: 0 }}
                  >
                    <TrackedLink
                      href={`/writing/${post.slug}`}
                      eventName="post_click"
                      eventData={{ slug: post.slug, title: post.title }}
        ...
        <ul style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
          ...
          >
            <TrackedLink
              ...
              eventData={{ label: 'bułka.com', location: 'writing' }}
              external
              className={quickLinkClassName}
            >
              bułka.com
